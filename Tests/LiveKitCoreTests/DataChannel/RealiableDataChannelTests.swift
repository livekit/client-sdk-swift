...
@Suite(.serialized, .tags(.dataChannel, .e2e)) final class RealiableDataChannelTests: @unchecked Sendable {
    ...
    ///
    /// The timeout is on *idle*, not on the whole wait. This ships ~3.9 MB of reliable payload and
    /// a loaded runner can still be draining it well past any flat window; stopping early tears the
    /// rooms down with packets still in the transport, and those are lost — `send` resolves when a
    /// write reaches `sendData`, not when it is delivered. Resetting the deadline on progress keeps
    /// a genuine stall failing within `idleTimeout` while letting a slow drain finish.
    ///
    /// Waits for the *last* index rather than a full count: a reconnect may legitimately drop the
    /// in-flight window, where waiting for every index would always burn the whole timeout.
    ...
    private func expectDelivery(_ received: [UInt32], mode: ReconnectMode, iterations: Int) {
        ...
        default:
            // Deliberately not asserting zero loss across a reconnect. `startReconnect` escalates
            // to a *full* reconnect when the resume does not land in time, and a full reconnect
            // clears the publisher's replay set by design: `ReliableStage.reset()` drops the
            // retained writes along with the sequence counter they were stamped under, because
            // writes from the old counter cannot be replayed into a session whose counter
    ...
    func reliableDelivery(mode: ReconnectMode) async throws {
        ...
        // (see `expectDelivery`), so an exact count here would fail the modes that are working as
        // designed. The assertions after the block carry the real checks.
