...
struct GenerateFacades: ParsableCommand {
    ...
    func run() throws {
        ...
        let used = prune ? Self.usedSymbols() : nil
        let keep = used.map { Reachability(files: files, namer: namer).closure(from: $0.types) }

        let outFolder = try Folder(path: output)
        var stats = Stats()
        for file in files {
            let emitter = Emitter(namer: namer, usedMembers: used?.members)
        ...
        fields   : \(stats.fields)
        oneofs   : \(stats.oneofs)
        emitted  : \(emitted) types (prune \(prune ? "on" : "off"), member allowlist \(used?.members.count ?? 0) names)
        output   : \(output)
        """)
    }

    /// Symbols spelled anywhere in hand-written SDK or test code: facade type
    /// names, and member names (any identifier after a `.`). Over-collection
    /// from non-proto code is harmless — it only keeps an accessor alive; a
    /// missing member is a compile error, never silent.
    static func usedSymbols() -> (types: Set<String>, members: Set<String>) {
        ...
        for root in ["Sources/LiveKit", "Tests"] {
            ...
            for file in folder.files.recursive where file.name.hasSuffix(".swift") {
                ...
                for match in typePattern.matches(in: text, range: range) {
                    if let r = Range(match.range(at: 1), in: text) { types.insert(String(text[r])) }
                }
                for match in memberPattern.matches(in: text, range: range) {
                    if let r = Range(match.range(at: 1), in: text) { members.insert(String(text[r])) }
                }
                // Files extending a facade type reference members through
                // implicit self (no dot) — keep every identifier they spell.
                if text.contains("extension Livekit_") || text.contains("extension Google_Protobuf_") {
                    let identifier = try! NSRegularExpression(pattern: #"\b([a-z_][A-Za-z0-9_]*)\b"#)
                    for match in identifier.matches(in: text, range: range) {
                        if let r = Range(match.range(at: 1), in: text) { members.insert(String(text[r])) }
                    }
                }
            }
...
struct Emitter {
    let namer: SwiftProtobufNamer
    /// When non-nil, only accessors whose name appears in this set are emitted
    /// (map entries are exempt: generated map code needs their key/value).
    let usedMembers: Set<String>?

    private func wanted(_ name: some StringProtocol) -> Bool {
        guard let usedMembers else { return true }
        return usedMembers.contains(String(name).replacingOccurrences(of: "`", with: ""))
    }
    ...
    func emit(_ message: Descriptor, depth: Int) -> String {
        ...
        """
        // Map entries are exempt from member pruning: generated map accessors
        // on the parent use entry.key / entry.value.
        let emitter = message.options.mapEntry
            ? Emitter(namer: namer, usedMembers: nil) : self
        ...
        {
            let lines = emitter.emit(field, of: message)
        ...
        for oneof in Self.realOneofs(of: message) {
            let lines = emitter.emit(oneof: oneof, of: message)
            guard !lines.isEmpty else { continue }
            for line in lines {
                out += "\(pad)    \(line)\n"
            }
    ...
    private func emit(_ field: FieldDescriptor, of _: Descriptor) -> [String] {
        ...
        case .string:
            ...
            if wanted(names.has) {
                chunks.append(["var \(names.has): Bool { \(slot) != nil }"])
            }
            if wanted("with\(capitalised)Bytes") {
                chunks.append([
    ...
    private func emitRepeated(_ field: FieldDescriptor) -> [String] {
        ...
        guard wanted(names.name) else { return [] }
    ...
    private func emit(oneof: OneofDescriptor, of message: Descriptor) -> [String] {
        ...
        // Whole block keyed on any use of the oneof property or a variant
        // accessor; the enum and _clear helper come along whenever it's kept.
        guard wanted(property) || variants.contains(where: { wanted(caseName($0)) }) else {
            return []
        }
        ...
        for variant in variants where wanted(caseName(variant)) {
