...
extension Emitter {
    /// proto3 enums are open — a peer may send a value this build has never
    /// heard of — so they are emitted as a `RawRepresentable` struct rather
    /// than a Swift enum with an `UNRECOGNIZED` payload. Unknown values then
    /// round-trip as themselves instead of needing a case to carry them, and
    /// the switch-based `rawValue`/`init?(rawValue:)` pair disappears.
    ///
    /// `case .reliable` still pattern-matches (through `~=` on `Equatable`);
    /// what changes is that a `switch` needs a `default`, which an open enum
    /// always did semantically.
    func emit(enum enumType: EnumDescriptor, depth _: Int = 0) -> String {
        ...
        var out = "struct \(type): NanopbEnum {\n"
        out += "    let rawValue: Int\n"
        out += "    init(rawValue: Int) { self.rawValue = rawValue }\n"
        out += "    init() { self.init(rawValue: \(values.first?.number ?? 0)) }\n\n"
        for (value, name) in zip(values, names) {
            out += "    static let \(name) = Self(rawValue: \(value.number))\n"
        }
        out += "}\n\n"
        return out
    }
