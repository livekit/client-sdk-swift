...
extension Room: EngineDelegate {
        ...
        if state.connectionState != oldState.connectionState {
            ...
            if case .connected = state.connectionState {
                ...
                if case .reconnecting = oldState.connectionState {
                    delegates.notifyDetached { $0.roomDidReconnect?(self) }
                } else {
                    delegates.notifyDetached { $0.roomDidConnect?(self) }
                }
            } else if case .reconnecting = state.connectionState {
                // Re-connecting
                delegates.notifyDetached { $0.roomIsReconnecting?(self) }
            } else if case .disconnected = state.connectionState {
                // Disconnected
                if case .connecting = oldState.connectionState {
                    delegates.notifyDetached { $0.room?(self, didFailToConnectWithError: oldState.disconnectError) }
                } else {
                    delegates.notifyDetached { $0.room?(self, didDisconnectWithError: state.disconnectError) }
                }
            }
        }
    ...
    func engine(_ engine: Engine, didReceiveUserPacket packet: Livekit_UserPacket) async {
        ...
        engine.executeIfConnected { [weak self] in
            ...
            if let participant {
                participant.delegates.notifyDetached { [weak participant] delegate in
