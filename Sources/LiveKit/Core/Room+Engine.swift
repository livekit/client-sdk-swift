...
extension Room {
    ...
    func send(dataPacket packet: consuming Livekit_DataPacket) async throws {
        // A room that is disconnected, or on its way there, has no channel left to attach to, and
        // a drain parks a submitted write until one arrives — so without this the send never
        // settles. `.connecting` / `.reconnecting` still park by design: the write resumes on the
        // channel that session brings up.
        guard !_state.connectionState.isTearingDown else {
