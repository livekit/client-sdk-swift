...
public enum ScreenShareSource {
    ...
    extension MacOSScreenCapturer {
        static func computeCaptureMethod(preferredMethod: MacOSScreenCapturePreferredMethod) -> MacOSScreenCaptureMethod {
    ...
    public class MacOSScreenCapturer: VideoCapturer {
        ...
        private lazy var session: AVCaptureSession = {
            assert(captureMethod == .legacy, "Should be only executed for legacy mode")
        ...
        {
            // compatibility
            captureSource = source.toScreenCaptureSource()
            self.options = options
            captureMethod = Self.computeCaptureMethod(preferredMethod: preferredMethod)
        ...
        private func onDispatchSourceTimer() {
            ...
            guard case .started = captureState,
        ...
        override public func stopCapture() -> Promise<Bool> {
            func createStopPromise() -> Promise<Bool> {
                Promise<Bool>(on: queue) { fulfill, reject in

                    if #available(macOS 12.3, *), case .screenCaptureKit = self.captureMethod {
                        guard let stream = self._scStream as? SCStream else {
                            assertionFailure("SCStream is nil")
                            fulfill(true)
                            return
                        }
    ...
    extension MacOSScreenCapturer {
        private func restartFrameResendTimer() {
            ...
            assert(captureMethod == .screenCaptureKit, "Should be only executed for .screenCaptureKit mode")
    ...
    extension ScreenShareSource {
        func toScreenCaptureSource() -> MacOSScreenCaptureSource {
            switch self {
            case let .window(id): return MacOSWindow(from: id)
            case let .display(id): return MacOSDisplay(from: id)
            }
        }
    }
    ...
    public extension MacOSScreenCapturer {
        ...
        static func mainDisplaySource() -> Promise<MacOSDisplay> {
        ...
        {
            Promise<[MacOSScreenCaptureSource]> { fulfill, reject in
        ...
        static func displaySources() -> Promise<[MacOSDisplay]> {
            sources(for: .display).then(on: queue) { sources in
                // cast
        ...
        static func windowSources() -> Promise<[MacOSWindow]> {
    ...
    public extension MacOSScreenCapturer {
        ...
        static func sources() -> [ScreenShareSource] {
            [displayIDs().map { ScreenShareSource.display(id: $0) },
             windowIDs().map { ScreenShareSource.window(id: $0) }].flatMap { $0 }
        }

        @available(*, deprecated, message: "Use sources(for:) instead")
        static func displayIDs() -> [CGDirectDisplayID] {
            _displayIDs()
        }

        @available(*, deprecated, message: "Use sources(for:) instead")
        static func windowIDs(includeCurrentProcess: Bool = false) -> [CGWindowID] {
        ...
        internal static func _windowIDs(includeCurrentProcess: Bool = false) -> [CGWindowID] {
            ...
            return list
                ...
                .map { $0.object(forKey: kCGWindowNumber) as? NSNumber }.compactMap { $0 }.map(\.uint32Value)
        }
    }
