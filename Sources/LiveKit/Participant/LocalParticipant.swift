...
public class LocalParticipant: Participant {
    ...
    internal func publish(track: LocalTrack,
        ...
        return track.start().then(on: queue) { _ -> Promise<Dimensions?> in
        ...
        }.then(on: queue) { dimensions -> Promise<(result: LK_RTCRtpTransceiverInit, trackInfo: Livekit_TrackInfo)> in
            ...
            self.room.engine.signalClient.sendAddTrack(cid: track.mediaTrack.trackId,
                ...
                let transInit = DispatchQueue.liveKitWebRTC.sync { LK_RTCRtpTransceiverInit() }
        ...
        }.then(on: queue) { (transInit, trackInfo) -> Promise<(transceiver: LK_RTCRtpTransceiver, trackInfo: Livekit_TrackInfo)> in
        ...
        }.then(on: queue) { params -> Promise<(LK_RTCRtpTransceiver, trackInfo: Livekit_TrackInfo)> in
