...
extension TelemetryOptions {
    ...
    /// cache lives). `storageDirectory: nil` keeps the cache in memory.
    var config: TelemetryConfig {
        TelemetryConfig(endpoint: endpoint?.absoluteString,
                        headers: headers,
                        ...
                        disabledInstruments: TelemetryInstruments.all.subtracting(instruments).lowered)
    }
}

extension TelemetryInstruments {
    var lowered: [Instrument] {
        [(Self.device, Instrument.device), (Self.logs, .logs), (Self.rtc, .rtc), (Self.room, .room)]
